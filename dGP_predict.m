function [df_star] = dGP_predict(x, y, x_star, k, cov_fun, loghyper, K_inv)
%--------------------------------------------------------------------------
%
% FUNCTION:		dGP_predict
%
% PURPOSE:		Make a Gaussian Process prediction to estimate mean
%				derivative with respect to kth input dimension at specified
%				points (x_star) from observations (x, y).
%               
% SYNTAX:		[df_star] = dGP_predict(x, y, x_star, k, cov_fun, loghyper)
%
%				To just provide inv(K) use empty test point set:
%				[Ki] = dGP_predict(x, y, [], cov_fun, loghyper)
%
%				To solve for a known Ki = inv(K):
%				[df] = dGP_predict(x, y, x_star, k, cov_fun, loghyper, Ki)
%
...
%
% See also:		GP_predict, GP_likelikood
%--------------------------------------------------------------------------
...
if nargin ~= 7
	...
	K = cov_fun{1}(x, x, loghyper);			% Training points covariances
...
if isempty(x_star)
	...
	if numel(k) > 1
		...
		for i = 1:numel(k)
			k_star = cov_fun(x_star, x, k(i), loghyper);	% Related covariances
			df_star(i,:,:) = (k_star*K_inv*y)';
		end
	else
		k_star = cov_fun(x_star, x, k, loghyper);	% Related covariances
